...
`;
        ...
        // Parse action cards (AI returns JSON array)
        let actions = [];
        try {
            const actionText = actionResponse.text.trim();
            const jsonMatch = actionText.match(/\[\s*\{[\s\S]*\}\s*\]/);
            actions = JSON.parse(jsonMatch ? jsonMatch[0] : actionText);
        } catch (parseErr) {
            console.error('⚠️ Failed to parse action JSON:', parseErr.message);
            actions = [
...
router.get('/thisMonth', async (req, res) => {
    try {
        ...
        // Actions for this month are generated by /lastMonth and saved under last month's key
        const now = new Date();
        const lastMonthKey = new Date(now.getFullYear(), now.getMonth() - 1, 1).toISOString().split('T')[0];

        console.log('Looking up actions from month key:', lastMonthKey);

        // Fetch actions from monthly_summary (saved during last month's analysis)
